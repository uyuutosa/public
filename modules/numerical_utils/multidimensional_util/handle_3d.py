...
#        self.ccor.dinput(arrs)
    ...
    def mov_grad(self, wid, abcs=0, lgtd=1):
        self.hist.input()
        from numpy import array, mean
        from decimal import Decimal
        arrs,errs = self.get_data(reterr=True)
        ...
        arrs[abcs] = x + wid / 2
        arrs[lgtd] = a[L-1:]
        arrs[3-abcs-lgtd] = z[L-1:] if not z is None else None
    ...
    def mov_var(self, wid, abcs=0, lgtd=1):
        ...
        var_y = convolve(h, (convolve(h, y)[:1-L] / L - y) ** 2 ) / L
    ...
    def slice_arr(self, slst, xyz=0):
        ...
        arrs = self.get_data()
        x = arrs[xyz]
        #input(mywhere((slst[0] <= x) == (x <= slst[1])))
        #input(mywhere((slst[0] <= x) == (x <= slst[1]))[0])
        args = mywhere((slst[0] <= x) == (x <= slst[1]))[0]
        if len(args) == 0: 
            return False
    ...
    def zero_adj(self, plst, abcs=0, lgtd=1, inv=True):
        ...
        x = arrs[abcs]
        y = arrs[lgtd]
        y -= y[mywhere((plst[0] <= x) == (x <= plst[1]))[0]].mean()
        arrs[abcs] = x
        arrs[lgtd] = y
        self.zadj = handle_3d()
    ...
    def punch_out(self, plst, pxyz=0, inv=False, val=None, vxyz=1):
        ...
        for pmin, pmax in plst: 
            ...
            #numarr = mywhere( inv == ((pmin <= dlst[pxyz]) * (pmax >= dlst[pxyz])))[0]
        if val is None:
...
#        from subprocess import call
    ...
    def slice_same(self, obj, xyz=0, err=0.001):
        self.hist.input()
        arrs = self.get_data()
        arrs2 = obj.get_data()
        x = arrs[xyz]
        ...
        #input(mywhere((slst[0] <= x) == (x <= slst[1])))
        #input(mywhere((slst[0] <= x) == (x <= slst[1]))[0])
        arrs = [arr[mywhere((slst[0]-err <= x) == (x <= slst[1]+err))[0]] if not arr is None else None for arr in arrs]
        self.slice = handle_3d()
        self.slice.dinput(arrs)
...
#        itvl_d = int(itvl / tstep)
    ...
    def filter_decimation(self, flst, abcs=0, lgtd=1):
        obj = deepcopy(self)
        obj.hist.input()    
        obj.fft_wm(abcs=abcs,lgtd=lgtd)
