...
#####################################    
    if RAW == 0:
        ...
        if len(Popen(["ls",environ["HOME"]+"/calc_data/"+avename],stdout=PIPE).stdout.read()) == 0:
            p=Popen([calc_cmd,"-s",shot,"-c",ch],stdin=PIPE)#, "-a", "-o","3.5,14.5"],stdin=PIPE,stdout=PIPE,stderr=PIPE)
...
#####################################    
    ...
    from modules.essential_utils.file_util import loaddat
    #dlst = loaddat(NAME, delim=",").transpose()
    dlst = loadtxt(NAME, delimiter=",").transpose()
    ret = [dlst, ["no","name"]]
...
def calc_mach(shot,itvl=0.01,wid=0.3,RAW=0, chs = ["Is_mach1", "Is_mach2"], name=r"$R_{\rm Mach}$", calib=1.24, vmach=None, Z=2, ti=5, num=None, mvelo=1):
    ...
    xy1=calc3("simple",str(shot),channel(chs[0],str(shot)),itvl,wid,RAW=RAW)
    xy2=calc3("simple",str(shot),channel(chs[1],str(shot)),itvl,wid,RAW=RAW)
    if vmach:
        mp = 1.672e-27
        c = 1.60217733e-19
        k = 1.38065e-23
...
def calc_mach_weak_magnetized(shot,itvl=0.01,wid=0.3,RAW=0, chs = ["Is_mach1", "Is_mach2"], name=r"$R_{\rm Mach}$", calib=1.24, vmach=None, Z=2, ti=5, num=None, mvelo=1):
    from numpy import array, log, sqrt
    ...
    tmp = alignment([xy1[0], xy2[0], te[0]])
    te = [tmp[0], tmp[2]]
    MPCR_data=[xy1[0][0],((calib*xy1[0][1]-xy2[0][1])/(calib*xy1[0][1]+xy2[0][1])) * sqrt(te[0][1]/2)]
    MPCR_tag=[xy1[1][0],name]
    MPCR=[MPCR_data,MPCR_tag]
    return MPCR
...
def calc_Mp(shot,itvl=0.01,ave=0.1,RAW=0, reff=None, iota=None):
    ...
    #Since the number of list is different between "triple" and "simple", their is balanced.
...
def calc(abbchname,shot,itvl=0.01,ave=0.03,RAW=0,e667=881737.765546,e728=2003465.38034, reff=None, iota=None, nom=None, expr_x=None, expr_y=None, timelst=None, f_name=None, vpp = 0.05, line=139e-3, width=0.01):
    """Calculate arbitrary parameter which is signal of raw voltage or physical parameter(calculated internal) and so on.
    ...
    Example
    -------
        Convinient assign for channel data example.
        calc("ex pc - 80001_pb", 80000)
    ...
    if abbchname == "mpcr":
        ...
        xy = ele_pressure(str(shot),itvl,ave,RAW=RAW)
...
#Mach probe area ---------------------------------
        ...
        #change 20140116####xy = calc_mach(str(shot),itvl,ave,RAW=RAW, chs= ["Ism_u1", "Ism_d1"], name=r"$R_{\rm Mach1}$", calib=0.856329850669, num=[27,28])
        #change 20131119xy = calc_mach(str(shot),int,ave,RAW=RAW, chs= ["Ism_u1", "Ism_d1"], name=r"$R_{\rm Mach1}$", calib=0.86084, num=[27,28])
        xy = calc_mach(str(shot),itvl, ave,RAW=RAW, chs= ["Ism_u1", "Ism_d1"], name=r"$R_{\rm Mach1}$", calib=0.85189832078193817)
    elif abbchname == "mpcr2":
        #xy = calc_mach(str(shot),int,ave,RAW=RAW, chs= ["Ism_u2", "Ism_d2"], name=r"$R_{\rm Mach2}$", calib=1, num=[29,30])
        #change 20140116xy = calc_mach(str(shot),itvl,ave,RAW=RAW, chs= ["Ism_u2", "Ism_d2"], name=r"$R_{\rm Mach2}$", calib=1.14898490437, num=[29,30])
        #change 20131119xy = calc_mach(str(shot),int,ave,RAW=RAW, chs= ["Ism_u2", "Ism_d2"], name=r"$R_{\rm Mach2}$", calib=1.1048, num=[29,30])
        xy = calc_mach(str(shot),itvl,ave,RAW=RAW, chs= ["Ism_u2", "Ism_d2"], name=r"$R_{\rm Mach2}$", calib=1.1096947078915167)
    elif abbchname == "mpcr3":
        #xy = calc_mach(str(shot),itvl,ave,RAW=RAW, chs= ["Ism_u3", "Ism_d3"], name=r"$R_{\rm Mach3}$", calib=1, num=[31,32])
        #change 20140116xy = calc_mach(str(shot),itvl,ave,RAW=RAW, chs= ["Ism_u3", "Ism_d3"], name=r"$R_{\rm Mach3}$", calib=1.04608681374, num=[31,32])
        #change 20131119xy = calc_mach(str(shot),int,ave,RAW=RAW, chs= ["Ism_u3", "Ism_d3"], name=r"$R_{\rm Mach3}$", calib=1.0076, num=[31,32])
        xy = calc_mach(str(shot),itvl,ave,RAW=RAW, chs= ["Ism_u3", "Ism_d3"], name=r"$R_{\rm Mach3}$", calib=1.0286, num=[31,32])
#Mach probe area ---------------------------------
    ...
    elif abbchname == "imach1":
        #xy = calc_mach(str(shot),int,ave,RAW=RAW, chs= ["Ism_u1", "Ism_d1"], name=r"$R_{\rm Mach1}$", calib=1, num=[27,28])
        xy = calc_mach_weak_magnetized(str(shot),itvl,ave,RAW=RAW, chs= ["Ism_u1", "Ism_d1"], name=r"$R_{\rm Mach1}$", calib=0.856329850669, num=[27,28])
        #change 20131119xy = calc_mach(str(shot),int,ave,RAW=RAW, chs= ["Ism_u1", "Ism_d1"], name=r"$R_{\rm Mach1}$", calib=0.86084, num=[27,28])
        #xy = calc_mach(str(shot),int,ave,RAW=RAW, chs= ["Ism_u1", "Ism_d1"], name=r"$R_{\rm Mach1}$", calib=0.85189832078193817)
    elif abbchname == "imach2":
        #xy = calc_mach(str(shot),int,ave,RAW=RAW, chs= ["Ism_u2", "Ism_d2"], name=r"$R_{\rm Mach2}$", calib=1, num=[29,30])
        xy = calc_mach_weak_magnetized(str(shot),itvl,ave,RAW=RAW, chs= ["Ism_u2", "Ism_d2"], name=r"$R_{\rm Mach2}$", calib=1.14898490437, num=[29,30])
        #change 20131119xy = calc_mach(str(shot),int,ave,RAW=RAW, chs= ["Ism_u2", "Ism_d2"], name=r"$R_{\rm Mach2}$", calib=1.1048, num=[29,30])
        #xy = calc_mach(str(shot),int,ave,RAW=RAW, chs= ["Ism_u2", "Ism_d2"], name=r"$R_{\rm Mach2}$", calib=1.1096947078915167)
    elif abbchname == "imach3":
        #xy = calc_mach(str(shot),itvl,ave,RAW=RAW, chs= ["Ism_u3", "Ism_d3"], name=r"$R_{\rm Mach3}$", calib=1, num=[31,32])
        xy = calc_mach_weak_magnetized(str(shot),itvl,ave,RAW=RAW, chs= ["Ism_u3", "Ism_d3"], name=r"$R_{\rm Mach3}$", calib=1.04608681374, num=[31,32])
        #change 20131119xy = calc_mach(str(shot),int,ave,RAW=RAW, chs= ["Ism_u3", "Ism_d3"], name=r"$R_{\rm Mach3}$", calib=1.0076, num=[31,32])
    elif abbchname == "<ne>":
...
#    def __init__(self, shot, itvl=0.01, wid=0.1, raw=False, stime=(26, 36)):
        ...
        xy=calc3("simple",str(shot),abbchan(str(abbchname),str(shot)),itvl,ave,RAW=RAW)
    if nom != None:
...
#        else :
#            xy=calc3("simple",str(shot),abbchan(str(abbchname),str(shot)),int,ave,RAW=RAW)
#        xys += [xy]
#    print xys
